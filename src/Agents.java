...
public class Agents { //Agents sera abstract, avec différents types d'agents
	private boolean alive;
	private int age;
	private Image tmp; // A retirer quand il y aura des agents
	private int tailleX, tailleY;
	private int x, y;
	public Agents(int x, int y, int tailleX, int tailleY) {
		alive = true;
		age = 1;
		this.x = x;
		this.y = y;
		try { //A retirer quand il y aura des agents
			tmp = ImageIO.read(new File("cactus1.png"));
		} catch ( Exception e ) {
			e.printStackTrace();
			System.exit(-1);
		}
